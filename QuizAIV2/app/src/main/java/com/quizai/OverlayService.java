...
public class OverlayService extends Service {
    ...
    private void createChannel() {
        ...
        ch.setDescription("QuizAI läuft im Hintergrund");
    ...
    private Notification buildNotif() {
        ...
        return new NotificationCompat.Builder(this, CH)
            ...
            .setContentText("Grüner Button ist auf deinem Bildschirm")
    ...
    public class Bridge {
        ...
        public void setAnalyzing(boolean b) {
            ...
            new Handler(Looper.getMainLooper()).post(() -> {
                ...
                else if (panelOpen) setBubble("✕", "#FF4466");
                else setBubble("🤖", "#00E5A0");
            });
        }
    ...
    private String buildHTML() {
        ...
        ".htit{font-size:17px;font-weight:800}.htit b{color:#00E5A0}" +
        ".xbtn{width:32px;height:32px;background:#1C1C2E;border:1px solid #2A2A42;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:15px;cursor:pointer;text-align:center;line-height:32px}" +
        ...
        ".uico{font-size:30px}.utit{font-size:14px;font-weight:700}.usub{font-size:11px;color:#555577;line-height:1.5}" +
        ...
        ".rltr{font-size:54px;font-weight:900;color:#00E5A0;line-height:1;text-shadow:0 0 30px rgba(0,229,160,.6);flex-shrink:0}" +
        ".rtag{font-size:9px;color:#00E5A0;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:4px}" +
        ".rans{font-size:15px;font-weight:800;line-height:1.3;margin-bottom:4px}" +
        ...
        "</style></head><body>" +
        "<div class='bar'></div>" +
        "<div class='hdr'><div class='htit'>Quiz<b>AI</b> 🤖</div><div class='xbtn' onclick='Android.close()'>✕</div></div>" +
        "<div class='scr'>" +
        "<div class='modes'>" +
        ...
        "<div class='uico'>📷</div><div class='utit'>Kamera öffnen</div>" +
        "<div class='usub'>Richte sie auf die Quiz-Frage</div></div><img class='prev' id='ci'></div>" +
        "<div class='res' id='res'>" +
        "<div class='empty' id='emp'>📸 Bild laden → Analysieren</div>" +
        "<div class='rbody' id='rb'>" +
        "<div class='rtop'><div class='rltr' id='rl'>A</div>" +
        ...
        "{type:'text',text:'Analysiere diesen Quiz-Screenshot. Welche Antwort ist richtig? Antworte NUR als JSON ohne Backticks: {\"letter\":\"A\",\"answer\":\"Antworttext\",\"reason\":\"Warum auf Deutsch\",\"confidence\":90}'}" +
        "]}]})});" +
        "var d=await r.json();" +
    ...
    private void styleBubble(TextView tv, String color) {
        ...
        String stroke = color.equals("#00E5A0") ? "#00B87A" : color.equals("#FFA726") ? "#E65100" : "#CC0033";
        bg.setStroke(dp(2), Color.parseColor(stroke));
        tv.setBackground(bg);
    }

    private void setBubble(String emoji, String color) {
        new Handler(Looper.getMainLooper()).post(() -> {
            if (bubble instanceof TextView) {
                ((TextView) bubble).setText(emoji);
